...
## How to interact directly with the Eden Bundles RPC endpoint

For advanced users, you can interact with the relay's RPC endpoint at `https://api.edennetwork.io/v1/bundle`. The relay provides new JSON-RPC methods for interfacing with Eden which are documented below:

### eth_sendBundle

`eth_sendBundle` can be used to send your bundles to Eden builders. The `eth_sendBundle` RPC has the following payload format:

> :information_source: To make your bundle cancellable, you must pass `replacementUuid` when first submitting the bundle. Calling this method subsequent times with the same UUID will replace the original bundle. See [Bundle Cancellations](/eden-bundles/searchers/advanced/bundle-cancellations) for more information.
...
See [Bundle Cancellations](/eden-bundles/searchers/advanced/bundle-cancellations) for more information.
...
```curl
curl -X POST -H "Content-Type: application/json" -H "X-Flashbots-Signature: 0x1234:0xabcd" --data '{"jsonrpc":"2.0","method":"eth_sendBundle","params":[{see above}],"id":1}' https://api.edennetwork.io/v1/bundle
```
