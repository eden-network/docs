...
We ETL data from MEV-Boost Relays from the endpoint `/bids`, store and make the data available in BigQuery. Our dataset contains public information provided by each relay from their API.
* Relays: Flashbots, Eden, Ultrasound, Aestus, Agnostic, ...
* We augment data to show if blocks have been re-orged
* We augment block timestamp
...
* Table: `eden-data-public.mev_boost.bids`
